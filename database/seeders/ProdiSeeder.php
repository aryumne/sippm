<?php

namespace Database\Seeders;

use App\Models\Prodi;
use Illuminate\Database\Seeder;

class ProdiSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        Prodi::create([
            'faculty_id' => 1,
            'nama_prodi' => 'Matematika',
        ]);
        Prodi::create([
            'faculty_id' => 1,
            'nama_prodi' => 'Fisika',
        ]);
        Prodi::create([
            'faculty_id' => 1,
            'nama_prodi' => 'Biologi',
        ]);
        Prodi::create([
            'faculty_id' => 1,
            'nama_prodi' => 'Kimia',
        ]);
        Prodi::create([
            'faculty_id' => 2,
            'nama_prodi' => 'Pendidikan Matematika',
        ]);
        Prodi::create([
            'faculty_id' => 2,
            'nama_prodi' => 'Pendidikan Fisika',
        ]);
        Prodi::create([
            'faculty_id' => 2,
            'nama_prodi' => 'Pendidikan Biologi',
        ]);
        Prodi::create([
            'faculty_id' => 2,
            'nama_prodi' => 'Pendidikan Kimia',
        ]);
        Prodi::create([
            'faculty_id' => 2,
            'nama_prodi' => 'Pendidikan Bahasa Indonesia',
        ]);
        Prodi::create([
            'faculty_id' => 2,
            'nama_prodi' => 'Pendidikan Bahasa Inggris',
        ]);
        Prodi::create([
            'faculty_id' => 2,
            'nama_prodi' => 'Pendidikan Guru',
        ]);
        Prodi::create([
            'faculty_id' => 3,
            'nama_prodi' => 'Teknik Elektro',
        ]);
        Prodi::create([
            'faculty_id' => 3,
            'nama_prodi' => 'Teknik Informatika',
        ]);
        Prodi::create([
            'faculty_id' => 3,
            'nama_prodi' => 'Teknik Sipil',
        ]);
        Prodi::create([
            'faculty_id' => 3,
            'nama_prodi' => 'D3 Teknik Listrik',
        ]);
        Prodi::create([
            'faculty_id' => 3,
            'nama_prodi' => 'D3 Teknik Komputer',
        ]);
        Prodi::create([
            'faculty_id' => 4,
            'nama_prodi' => 'Teknik Pertambangan',
        ]);
        Prodi::create([
            'faculty_id' => 4,
            'nama_prodi' => 'Teknik Geologi',
        ]);
        Prodi::create([
            'faculty_id' => 4,
            'nama_prodi' => 'Teknik Perminyakan',
        ]);
        Prodi::create([
            'faculty_id' => 5,
            'nama_prodi' => 'Sastra Inggris',
        ]);
        Prodi::create([
            'faculty_id' => 5,
            'nama_prodi' => 'Sastra Indonesia',
        ]);
        Prodi::create([
            'faculty_id' => 5,
            'nama_prodi' => 'Antropologi',
        ]);
        Prodi::create([
            'faculty_id' => 6,
            'nama_prodi' => 'Teknologi Hasil Pertanian',
        ]);
        Prodi::create([
            'faculty_id' => 6,
            'nama_prodi' => 'Teknologi Pertanian dan Biosistem',
        ]);
        Prodi::create([
            'faculty_id' => 7,
            'nama_prodi' => 'Agroteknologi',
        ]);
        Prodi::create([
            'faculty_id' => 7,
            'nama_prodi' => 'Agribisnis',
        ]);
        Prodi::create([
            'faculty_id' => 7,
            'nama_prodi' => 'Ilmu Tanah',
        ]);
        Prodi::create([
            'faculty_id' => 7,
            'nama_prodi' => 'D3 Budidaya Tanaman Perkebunan',
        ]);
        Prodi::create([
            'faculty_id' => 7,
            'nama_prodi' => 'D3 Budidaya Tanaman Pangan',
        ]);
        Prodi::create([
            'faculty_id' => 8,
            'nama_prodi' => 'Manajemen Sistem Perairan',
        ]);
        Prodi::create([
            'faculty_id' => 8,
            'nama_prodi' => 'Ilmu Kelautan',
        ]);
        Prodi::create([
            'faculty_id' => 8,
            'nama_prodi' => 'Ekowisata',
        ]);
        Prodi::create([
            'faculty_id' => 9,
            'nama_prodi' => 'Peternakan',
        ]);
        Prodi::create([
            'faculty_id' => 9,
            'nama_prodi' => 'Nutrisi dan Teknologi Pakan Ternak',
        ]);
        Prodi::create([
            'faculty_id' => 9,
            'nama_prodi' => 'D3 Kesehatan Hewan',
        ]);
        Prodi::create([
            'faculty_id' => 9,
            'nama_prodi' => 'D3 Budidaya Ternak',
        ]);
        Prodi::create([
            'faculty_id' => 10,
            'nama_prodi' => 'Kehutanan',
        ]);
        Prodi::create([
            'faculty_id' => 10,
            'nama_prodi' => 'D3 Budidaya Hutan',
        ]);
        Prodi::create([
            'faculty_id' => 10,
            'nama_prodi' => 'D3 Koservasi Sumberdaya Hutan',
        ]);
        Prodi::create([
            'faculty_id' => 10,
            'nama_prodi' => 'D3 Manajemen Hutan Alam Produksi',
        ]);
        Prodi::create([
            'faculty_id' => 11,
            'nama_prodi' => 'Ekonomi Pembangunan',
        ]);
        Prodi::create([
            'faculty_id' => 11,
            'nama_prodi' => 'Manajemen',
        ]);
        Prodi::create([
            'faculty_id' => 11,
            'nama_prodi' => 'Akuntansi',
        ]);
        Prodi::create([
            'faculty_id' => 12,
            'nama_prodi' => 'Pendidikan Kedokteran',
        ]);
        Prodi::create([
            'faculty_id' => 13,
            'nama_prodi' => 'S2 Ilmu Pertanian',
        ]);
        Prodi::create([
            'faculty_id' => 13,
            'nama_prodi' => 'S3 Ilmu Lingkungan',
        ]);
        Prodi::create([
            'faculty_id' => 13,
            'nama_prodi' => 'S2 Biologi',
        ]);
        Prodi::create([
            'faculty_id' => 13,
            'nama_prodi' => 'S2 Sumberdaya Akuatik',
        ]);
        Prodi::create([
            'faculty_id' => 13,
            'nama_prodi' => 'S2 Kehutanan',
        ]);
        Prodi::create([
            'faculty_id' => 13,
            'nama_prodi' => 'S2 Ilmu Lingkungan',
        ]);

    }
}
