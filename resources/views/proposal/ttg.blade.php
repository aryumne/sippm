            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="material-datatables">
                        <table id="datatables-ttg" class="table table-striped table-no-bordered table-hover"
                                ...
                                <tr>
                                    ...
                                    <td class="text-right">
                                        ...
                                        <button type="button" class="btn btn-link btn-warning btn-just-icon edit"
                                            data-toggle="modal" data-target="#ubahTtg{{ $t->id }}">
                                            <i class="material-icons">mode_edit</i></a>
                                        </button>
                                        <!-- Ubah data menggunakan Modal -->
                                        <div class="modal fade" id="ubahTtg{{ $t->id }}" tabindex="-1" role="dialog"
                                            aria-labelledby="ubahTtg{{ $t->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form class="form" id="EditTtgValidation"
                                                        action="{{ route('ttg.update', $t->id) }}" method="POST"
                                                        enctype="multipart/form-data">
                                                        <div class="modal-body">
                                                            @csrf
                                                            @method('PUT')
                                                            <input type="hidden" name="path_ttg"
                                                                value="{{ $t->path_ttg }}">
                                                            ...
                                                            <div class="form-group mt-3">
                                                                ...
                                                                <input type="text" class="form-control datepicker"
                                                                    ...
                                                                    value="{{ now()->toDateString('Y-m-d') }}"
                                                                    value="{{ old('tanggal_upload', $t->tanggal_upload) }}"
                                                                    required>
                                                            </div>
                                                            ...
                                                            <div class="form-group pt-2">
                                                                <label for="bidang" class="bmd-label-floating">Bidang
                                                                    TTG</label>
                                                                <input type="text" class="form-control" id="bidang"
                                                                    name="bidang"
                                                                    value="{{ old('bidang', $t->bidang) }}" required>
                                                            </div>
                                                            @error('bidang')
                                                            <span id="category_id-error" class="error text-danger"
                                                                for="input-id" style="display: block;">{{ $message
                                        ...
                                        {{-- <a href="" class="btn btn-link btn-danger btn-just-icon remove">
                                            <form class="form" action="{{ route('ttg.destroy', $t->id) }}" method="POST"
                                                id="DeteleTtgValidation">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
...
<div class="modal fade" id="formTtg" tabindex="-1" role="dialog" aria-labelledby="formTtg" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form class="form" id="AddTtgValidation" action="{{ route('ttg.store') }}" method="POST"
                enctype="multipart/form-data">
                <div class="modal-body">
                    @csrf
