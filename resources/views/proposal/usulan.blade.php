                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-8">
                                        <a href="{{ asset('storage/' . $pps->path_proposal) }}" target="_blank" class="card-link text-rose">Download</a>
            ...
            <form class="form" id="AddProposalValidation" action="{{ route('usulan.store') }}" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="form-group mt-3">
                        <input type="text" class="form-control datepicker" id="tanggal_usul" name="tanggal_usul" placeholder="Tanggal Pengusulan" value="{{ now()->toDateString('Y-m-d') }}" value="{{ old('tanggal_usul') }}" required>
                    </div>
                    @error('tanggal_usul')
                    <span id="category_id-error" class="error text-danger" for="input-id" style="display: block;">{{ $message }}</span>
                    @enderror
                    <div class="form-group">
                        ...
                        <select class="form-control selectpicker" data-style="btn btn-link" id="nidn_pengusul" name="nidn_pengusul" required>
                            @foreach ($dosen as $ds)
                            <option value="{{ strlen($ds->nidn) <= 9 ? str_pad($ds->nidn, 10, '0', STR_PAD_LEFT) : $ds->nidn }}">
                                {{ $ds->nama }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select multiple class="form-control selectpicker" data-style="btn btn-link" id="nidn_anggota" name="nidn_anggota[]" required>
                            @foreach ($dosen as $ds)
                            <option value="{{ strlen($ds->nidn) <= 9 ? str_pad($ds->nidn, 10, '0', STR_PAD_LEFT) : $ds->nidn }}">
                                {{ $ds->nama }}
                            </option>
